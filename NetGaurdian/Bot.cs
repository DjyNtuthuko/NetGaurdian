    ...
    internal class Bot
    {
        ...
        public void Chat()
        {
            ...
            while (true)
            {
                ...
                {
                    TypeResponse("NetGuardian: Don't be shy, go ahead and ask me something!", ConsoleColor.Yellow);
                    continue;
                }

                string normalized = userInput.ToLower();
                history.Add("You: " + normalized);
                ShowTypingIndicator();

                // Check sentiment and commands first, then try to match a topic
                if (!HandleSentiment(normalized) && !HandleCommands(normalized))
                {
                    Topic matched = FindTopic(normalized);

                    if (matched != null)
                    {
                        // Pick a random response from the matched topic
                        string response = "NetGuardian: " + matched.Responses[rand.Next(matched.Responses.Length)];
                        TypeResponse(response, ConsoleColor.Green);
                        history.Add(response);

                        if (!string.IsNullOrEmpty(matched.FollowUp))
                            SuggestFollowUp(matched.FollowUp);
                    }
                    else
                    {
                        string fallback = "NetGuardian: Not sure about that one! Try asking about ransomware, breaches, firewalls, encryption, scams, backups or type 'tip'.";
                        TypeResponse(fallback, ConsoleColor.Yellow);
        ...
        // Handles greetings, frustration and general questions about the bot
        private bool HandleSentiment(string input)
        {
            if (Contains(input, "thank", "thanks", "appreciate"))
            {
                TypeResponse("NetGuardian: Anytime! Stay protected out there.", ConsoleColor.Green);
                return true;
            }

            if (Contains(input, "how are you", "how r you", "you good"))
            {
                TypeResponse("NetGuardian: Doing great! Ready to help you stay one step ahead online.", ConsoleColor.Green);
                return true;
            }

            if (Contains(input, "frustrated", "useless", "hate", "stupid"))
            {
                TypeResponse("NetGuardian: Sorry about that! Try asking me about ransomware, firewalls or scams.", ConsoleColor.Yellow);
                return true;
            }

            if (Contains(input, "purpose", "what do you do", "what can you do"))
            {
                TypeResponse("NetGuardian: I'm here to break down cybersecurity in plain language. No jargon, just useful advice!", ConsoleColor.Green);
                return true;
            }
        ...
        // Handles tip, history and exit commands
        private bool HandleCommands(string input)
        {
            if (Contains(input, "tip", "tips", "advice"))
            {
                string[] tips =
                {
                    "Back up your files at least once a week.",
                    ...
                    "Think before clicking any link, even from people you know.",
                    "Use a password manager to stay on top of your credentials.",
                    "Enable 2FA on every account that supports it.",
                    "Avoid oversharing personal info on social media."
                };

                // Pick a random tip from the array
                string tip = "NetGuardian: " + tips[rand.Next(tips.Length)];
            ...
            if (Contains(input, "exit", "quit", "bye", "later", "close", "goodbye"))
            {
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("\nNetGuardian: Catch you later! Stay safe and stay smart.");
                ExitArt();
                Environment.Exit(0);
                return true;
            }
        ...
        // Loops through all topics and returns the first keyword match
        private Topic FindTopic(string input)
        {
        ...
        // Returns true if the input contains any of the given keywords
        ...
        // Simulates the bot typing before a response appears
        private void ShowTypingIndicator()
        {
            Console.ForegroundColor = ConsoleColor.DarkGray;
            Console.Write("\nNetGuardian is typing");
        ...
        // Prints a response one character at a time
        ...
        // Suggests a related topic after a response
        ...
        private void Menu(string name)
        {
            ...
            Console.WriteLine(@"
                ...
                |                                                              |
                |                    NETGUARDIAN v1.0                         |
                |              Your Friendly Cyber Safety Buddy               |
                |                                                              |
                +--------------------------------------------------------------+
                |                                                              |
                ...
                |                                                              |
                |    * Ransomware             * Firewalls                     |
                |    * Data Breaches          * Safe Browsing                 |
                |    * Identity Theft         * The Dark Web                  |
                |    * Encryption             * Network Security              |
                |    * Scams & Fraud          * Passwords & 2FA               |
                |    * Antivirus              * Backups                       |
                |                                                              |
                +--------------------------------------------------------------+
                |                                                              |
                |   Commands:                                                  |
                |    tip      - get a quick cybersecurity tip                  |
                |    history  - see your chat history                          |
                |    exit     - leave the chat (or: bye, quit, later, close)   |
                |                                                              |
                +--------------------------------------------------------------+
");
            Console.ResetColor();
        ...
        private void ExitArt()
        {
            ...
            Console.WriteLine(@"
                ...
                |                    SEE YOU NEXT TIME!                       |
                |                                                              |
                |                Thanks for using NetGuardian                 |
                |                                                              |
                |           The internet is safer with people like you        |
                |                                                              |
                +--------------------------------------------------------------+
");
            Console.ResetColor();
